        ...
        fetchOperation.start { error, fetchedFolders, arg  in
            if let error = error {
                ...
                if let settings = (UIApplication.shared.delegate as? AppDelegate)?.settings {
                    let user = Address(name: self.nameTextField!.text!, address: session.username)!
                            ...
                            OperationQueue.main.addOperation {
                                if let authState = authState {
                                    if
                                        ...
                                        newSettings.isValid
                                    {
                                        _ = newSettings.save(toUserDefaults: .standard)
                                    }
                ...
                DispatchQueue.main.async {
                        ...
                        let email = json["email"] as? String
                    {
                        ...
                        if
                            ...
                            let address = Address(name: name, givenName: given, familyName: family, address: email)
                        {
                            OperationQueue.main.addOperation {
